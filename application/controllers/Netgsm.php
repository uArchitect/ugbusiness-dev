...
{     
    ...
    try {
        date_default_timezone_set('Europe/Istanbul');
        $all_records = []; // Tüm kayıtları birleştirmek için boş bir dizi oluştur

        $ayar = $this->Ayar_model->get_by_id(1); 
        
        // Son 7 günün verilerini çekmek için bir döngü
        for ($i = 7; $i >= 1; $i--) {
            $one_day_before = strtotime("-$i day");
            ...
            // Eğer veriler mevcutsa, ana diziye ekle
            if (isset($send_acc['records']) && is_array($send_acc['records'])) {
                $all_records = array_merge($all_records, $send_acc['records']);
            }
        }
        
        $viewData["santral_kayitlar"] = $all_records; // Birleştirilmiş kayıtları view'e gönder
