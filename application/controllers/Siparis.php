	...
	public function tamamlanmayanlar_ajax() { 
	   ...
	   $query = $this->db
        ...
        foreach ($query->result() as $row) {
            ...
            $data[] = [
                "<b>".$row->siparis_kodu."</b><br><span style='font-weight:normal'>".date('d.m.Y H:i',strtotime($row->kayit_tarihi))."<br>".date('d.m.Y H:i',strtotime($row->kurulum_tarihi))."</span>",
                "<b>".$musteri."</b>"."<br>"."<span style='font-weight:normal'>İletişim : ".formatTelephoneNumber($row->musteri_iletisim_numarasi)."</span><br>".($row->adim_no>11 ? " <i class='fas fa-check-circle text-success'></i><span class='text-success'>Teslim Edildi</span>":(($gun>0) ? '<span style="margin-left:10px;color:red;opacity:1">('.$gun.' gün önce)</span><br>'.((date(strtotime($row->kayit_tarihi)) < date(strtotime($row->kurulum_tarihi))) ? ('<span style="margin-left:10px;color:green;opacity:1">(Belirlenen Kurulum Tarihi : '.$tgun.' )</span>') : "") : '<span class="text-success"> (Bugün oluşturuldu)</span>'))."<br><span class='badge badge-warning ".($row->siparis_gorusme_aciklama == ""?"d-none":"")."'>".$row->siparis_gorusme_aciklama."</span>", 
