	   ...
	   $query = $this->db
        ...
        foreach ($query->result() as $row) {
            ...
            $data[] = [
				...
				"<b>".$row->merkez_adi."</b><span style='font-weight:normal'> / ".$row->sehir_adi." (".$row->ilce_adi.")"."</span><br>".(($row->merkez_adresi == "" || $row->merkez_adresi == "." || $row->merkez_adresi == "0") ? '<span style="opacity:0.4;font-weight:normal">BU MERKEZE TANIMLI ADRES KAYDI BULUNAMADI</span>' : "<span style='font-weight:normal'>".substr($row->merkez_adresi,60)."...</span>"),
