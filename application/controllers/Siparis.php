	   ...
	   $query = $this->db
        ...
        foreach ($query->result() as $row) {
            ...
            $data[] = [
                ...
                "<b>".$musteri."</b>".($row->adim_no>11 ? " <i class='fas fa-check-circle text-success'></i><span class='text-success'>Teslim Edildi</span>":(($gun>0) ? '<span style="margin-left:10px;color:red;opacity:1">('.$gun.' gün önce oluşturuldu)</span>' : '<span class="text-success">(Bugün oluşturuldu)</span>'))."<br>"."<span style='font-weight:normal'>İletişim : ".formatTelephoneNumber($row->musteri_iletisim_numarasi)."</span>", 
