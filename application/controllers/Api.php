			...
			if(count($kquery)>=0){
				...
				if(count($query)>=0){
					$json_data = [
						...
						"waitCount" => count($this->db->where("istek_durum_no",2)->where("istek_yonetici_id",$kquery[0]->kullanici_id)->or_where("istek_yonetici_id",$kquery[0]->kullanici_id)->get("istekler")->result()), 
						"processCount" => count($this->db->where("istek_durum_no",3)->where("istek_yonetici_id",$kquery[0]->kullanici_id)->or_where("istek_yonetici_id",$kquery[0]->kullanici_id)->get("istekler")->result()),
						"completedCount" => count($this->db->where("istek_durum_no",4)->where("istek_yonetici_id",$kquery[0]->kullanici_id)->or_where("istek_yonetici_id",$kquery[0]->kullanici_id)->get("istekler")->result()),  
						"data" => $query
					];
