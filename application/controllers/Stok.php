...
public function stok_cikis_yap()
{
    ...
    $control = $this->db->select('stoklar')->from("stoklar")->where(["sh.stok_seri_kod" => str_replace(" ","",escape($this->input->post('cikis_yapilacak_seri_kod')))])->get()->result();
    if (count($control) <= 0) {

   $control = $this->db->select('stoklar')->from("stoklar")->where(["sh.stok_seri_kod" => "01.034/LM".str_replace(" ","",escape($this->input->post('cikis_yapilacak_seri_kod')))])->get()->result();
