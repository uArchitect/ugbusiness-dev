        ...
        $query = $this->Stok_model->get_stok_kayitlari(["stok_seri_kod"=>$this->input->post('parca_seri_numarasi'),"stok_cikis_yapildi"=>1]) ;    
                if (count($query) > 0) {
                    ...
                    if($query[0]->anakart_onayi_verildi_mi == 1){
                        ...
                        $this->db->where("stok_seri_kod",$this->input->post('parca_seri_numarasi'))->update("stoklar",["anakart_onayi_verildi_mi"=>1,"anakart_onay_tarihi"=>date("Y-m-d H:i:s")]);
...
public function get_stok_kayitlari_ajax() {
    ...
    $alt_urunler = $this->Stok_model->stok_kayitlari_all(["stok_ust_grup_kayit_no"=>$stok_tanim->stok_id]);
        ...
        $row['stok_seri_kod'] = "<span style='cursor: pointer;' title='Güncellemek İçin Tıklayınız' onclick='serikoddegistir(\"$stok_tanim->stok_id\",\"$stok_tanim->stok_seri_kod\");' >".$stok_tanim->stok_seri_kod."</span>" ?: "<span style='opacity:0.5;'>Seri Kod Tanımlanmadı</span>";
...
if(count($alt_urunler)>0){
    foreach ($alt_urunler as $stok_tanim_alt) {
        ...
        $row['stok_seri_kod'] = "<span style='cursor: pointer;' title='Güncellemek İçin Tıklayınız' onclick='serikoddegistir(\"$stok_tanim_alt->stok_id\",\"$stok_tanim_alt->stok_seri_kod\");' >".$stok_tanim_alt->stok_seri_kod."</span>" ?: "<span style='opacity:0.5;'>Seri Kod Tanımlanmadı</span>";
