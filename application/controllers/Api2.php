...
class Api2 extends CI_Controller
{
    ...
    /** 43. Şirket İçi Kurallar (Dokuman) - Liste */
    public function sirket_ici_kurallar()
    {
        $method = $this->input->method(true);
        ...
        // Controller Referansı: Dokuman.php::index() (satır 59-72)
        // Model Referansı: Dokuman_model::get_all()
        // NOT: Dokuman ekleme/güncelleme/silme işlemleri web uygulamasından yapılmalıdır.
        ...
        $where = null;
        if ($kategori_id) {
            $where = ['dokuman_kategori_no' => $kategori_id];
        }
        
        $dokumanlar = $this->Dokuman_model->get_all($where);

        // Verileri formatla
        $formatted_data = [];
        foreach ($dokumanlar as $dokuman) {
            $formatted_data[] = [
                'dokuman_id' => intval($dokuman->dokuman_id),
                'dokuman_adi' => $dokuman->dokuman_adi ?? null,
                'dokuman_aciklama' => $dokuman->dokuman_aciklama ?? null,
                'dokuman_belge_no' => $dokuman->dokuman_belge_no ?? null,
                'dokuman_kategori_no' => !empty($dokuman->dokuman_kategori_no) ? intval($dokuman->dokuman_kategori_no) : null,
                'dokuman_yururluk_tarihi' => $dokuman->dokuman_yururluk_tarihi ?? null,
                'dokuman_guncelleme_tarihi' => $dokuman->dokuman_guncelleme_tarihi ?? null,
                'dokuman_sorumlu_kullanici_id' => !empty($dokuman->dokuman_sorumlu_kullanici_id) ? intval($dokuman->dokuman_sorumlu_kullanici_id) : null,
                'sorumlu_kullanici_adi' => $dokuman->kullanici_ad_soyad ?? null
            ];
        }

        $this->jsonResponse([
            'status' => 'success',
            'message' => 'Şirket içi kurallar başarıyla getirildi.',
            'data' => $formatted_data,
            'toplam_kayit' => count($formatted_data),
            'not' => 'Dokuman ekleme, güncelleme ve silme işlemleri web uygulamasından yapılmalıdır.',
            'timestamp' => date('Y-m-d H:i:s')
        ]);
    }

    /** 44. Şirket İçi Kurallar (Dokuman) - Detay */
    public function sirket_ici_kural_detay()
    {
        $method = $this->input->method(true);
        ...
        // Controller Referansı: Dokuman.php::edit() (satır 85-103)
        // Model Referansı: Dokuman_model::get_by_id()

        // Dokuman ID'sini al
        $dokuman_id = !empty($input_data['dokuman_id']) ? intval($input_data['dokuman_id']) : null;
        
        if (empty($dokuman_id)) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'dokuman_id gereklidir.'
            ], 400);
        }

        $this->load->model('Dokuman_model');
        $dokuman = $this->Dokuman_model->get_by_id($dokuman_id);
        
        if (!$dokuman || empty($dokuman)) {
            $this->jsonResponse([
                'status'  => 'error',
        ...
        $dokuman_data = $dokuman[0];

        // Revizyonları al
        $revizyonlar = $this->db
            ->where('revizyon_dokuman_no', $dokuman_id)
            ->order_by('revizyon_id', 'DESC')
            ->get('dokuman_revizyonlari')
            ->result();

        // İncelemeleri al
        $incelemeler = $this->db
            ->select('dokuman_incelemeleri.*, kullanicilar.kullanici_ad_soyad')
            ->from('dokuman_incelemeleri')
            ->join('kullanicilar', 'kullanicilar.kullanici_id = dokuman_incelemeleri.inceleme_sorumlu_kullanici_id', 'left')
            ->where('inceleme_dokuman_no', $dokuman_id)
            ->order_by('inceleme_id', 'DESC')
            ->get()
            ->result();

        // Revizyonları formatla
        $formatted_revizyonlar = [];
        foreach ($revizyonlar as $revizyon) {
            $formatted_revizyonlar[] = [
                'revizyon_id' => intval($revizyon->revizyon_id),
                'revizyon_kodu' => $revizyon->revizyon_kodu ?? null,
                'revizyon_aciklama' => $revizyon->revizyon_aciklama ?? null,
                'revizyon_dosya_adi' => $revizyon->revizyon_dosya_adi ?? null,
                'revizyon_dosya_url' => $revizyon->revizyon_dosya_adi ? base_url('uploads/' . $revizyon->revizyon_dosya_adi) : null,
                'revizyon_sorumlu_kullanici_id' => !empty($revizyon->revizyon_sorumlu_kullanici_id) ? intval($revizyon->revizyon_sorumlu_kullanici_id) : null
            ];
        }

        // İncelemeleri formatla
        $formatted_incelemeler = [];
        foreach ($incelemeler as $inceleme) {
            $formatted_incelemeler[] = [
                'inceleme_id' => intval($inceleme->inceleme_id),
                'inceleme_sorumlu_kullanici_id' => !empty($inceleme->inceleme_sorumlu_kullanici_id) ? intval($inceleme->inceleme_sorumlu_kullanici_id) : null,
                'sorumlu_kullanici_adi' => $inceleme->kullanici_ad_soyad ?? null
            ];
        }

        $this->jsonResponse([
            'status' => 'success',
            'message' => 'Dokuman detayı başarıyla getirildi.',
            'data' => [
                'dokuman_id' => intval($dokuman_data->dokuman_id),
                'dokuman_adi' => $dokuman_data->dokuman_adi ?? null,
                'dokuman_aciklama' => $dokuman_data->dokuman_aciklama ?? null,
                'dokuman_belge_no' => $dokuman_data->dokuman_belge_no ?? null,
                'dokuman_kategori_no' => !empty($dokuman_data->dokuman_kategori_no) ? intval($dokuman_data->dokuman_kategori_no) : null,
                'dokuman_yururluk_tarihi' => $dokuman_data->dokuman_yururluk_tarihi ?? null,
                'dokuman_guncelleme_tarihi' => $dokuman_data->dokuman_guncelleme_tarihi ?? null,
                'dokuman_sorumlu_kullanici_id' => !empty($dokuman_data->dokuman_sorumlu_kullanici_id) ? intval($dokuman_data->dokuman_sorumlu_kullanici_id) : null,
                'sorumlu_kullanici_adi' => $dokuman_data->kullanici_ad_soyad ?? null,
                'revizyonlar' => $formatted_revizyonlar,
                'incelemeler' => $formatted_incelemeler
            ],
            'not' => 'Dokuman ekleme, güncelleme ve silme işlemleri web uygulamasından yapılmalıdır.',
            'timestamp' => date('Y-m-d H:i:s')
        ]);
    ...
    public function fabrika_zimmet()
    {
        ...
        // Controller Referansı: Zimmet.php::index() (satır 380-423)
        // Tablolar: zimmet_stoklar, zimmet_hareketler, zimmet_departmanlar

        // Departman ID filtresi (opsiyonel)
        $departman_id = !empty($input_data['departman_id']) ? intval($input_data['departman_id']) : null;
        
        // Kullanıcı ID filtresi (opsiyonel)
        $kullanici_id = !empty($input_data['kullanici_id']) ? intval($input_data['kullanici_id']) : null;

        // Stok tanımları
        $stoklar = $this->db->order_by('zimmet_stok_adi', 'ASC')->get('zimmet_stoklar')->result();
        ...
        if ($departman_id) {
            $this->db->where('zh.zimmet_departman_no', $departman_id);
        }
        
        if ($kullanici_id) {
            $this->db->where('zh.zimmet_kullanici_no', $kullanici_id);
        }
        
        $this->db->order_by('zs.zimmet_stok_adi', 'ASC');
        $hareketler_detay = $this->db->get()->result();

        // Özet (toplam giriş/çıkış/kalan)
        $this->db->select('
            zs.*,
            zh.*,
            d.*,
        ...
        $this->db->order_by('zs.zimmet_stok_adi', 'ASC');
        $hareketler_ozet = $this->db->get()->result();

        // Stokları formatla
        $formatted_stoklar = [];
        foreach ($stoklar as $stok) {
            $formatted_stoklar[] = [
                'zimmet_stok_id' => intval($stok->zimmet_stok_id),
                'zimmet_stok_adi' => $stok->zimmet_stok_adi ?? null
            ];
        }

        // Hareketleri formatla
        $formatted_hareketler = [];
        foreach ($hareketler_detay as $hareket) {
            $formatted_hareketler[] = [
                'zimmet_hareket_id' => intval($hareket->zimmet_hareket_id),
                'zimmet_stok' => [
                    'zimmet_stok_id' => intval($hareket->zimmet_stok_id),
                    'zimmet_stok_adi' => $hareket->zimmet_stok_adi ?? null
                ],
                'zimmet_departman' => [
                    'zimmet_departman_id' => !empty($hareket->zimmet_departman_id) ? intval($hareket->zimmet_departman_id) : null,
                    'zimmet_departman_adi' => $hareket->zimmet_departman_adi ?? null
                ],
                'zimmet_kullanici_no' => !empty($hareket->zimmet_kullanici_no) ? intval($hareket->zimmet_kullanici_no) : null,
                'kullanici_adi' => $hareket->kullanici_ad_soyad ?? null,
                'zimmet_hareket_giris_miktar' => !empty($hareket->zimmet_hareket_giris_miktar) ? intval($hareket->zimmet_hareket_giris_miktar) : 0,
                'zimmet_hareket_cikis_miktar' => !empty($hareket->zimmet_hareket_cikis_miktar) ? intval($hareket->zimmet_hareket_cikis_miktar) : 0
            ];
        }

        // Özeti formatla
        $formatted_ozet = [];
        foreach ($hareketler_ozet as $ozet) {
            $formatted_ozet[] = [
                'zimmet_stok' => [
                    'zimmet_stok_id' => intval($ozet->zimmet_stok_id),
                    'zimmet_stok_adi' => $ozet->zimmet_stok_adi ?? null
                ],
                'zimmet_departman' => [
                    'zimmet_departman_id' => !empty($ozet->zimmet_departman_id) ? intval($ozet->zimmet_departman_id) : null,
                    'zimmet_departman_adi' => $ozet->zimmet_departman_adi ?? null
                ],
                'toplam_giris' => !empty($ozet->toplam_giris) ? intval($ozet->toplam_giris) : 0,
                'toplam_cikis' => !empty($ozet->toplam_cikis) ? intval($ozet->toplam_cikis) : 0,
                'kalan' => !empty($ozet->kalan) ? intval($ozet->kalan) : 0
            ];
        }

        $this->jsonResponse([
            'status' => 'success',
            'message' => 'Fabrika zimmet bilgileri başarıyla getirildi.',
            'data' => [
                'stoklar' => $formatted_stoklar,
                'hareketler' => $formatted_hareketler,
                'ozet' => $formatted_ozet
            ],
            'filtreler' => [
            ...
            'toplam_stok' => count($formatted_stoklar),
            'toplam_hareket' => count($formatted_hareketler),
            'timestamp' => date('Y-m-d H:i:s')
        ]);
    }

    /** 46. Fabrika Zimmet - Ekle */
    public function fabrika_zimmet_ekle()
    {
        $method = $this->input->method(true);
        ...
        // Controller Referansı: Zimmet.php::departmana_stok_tanimla() (satır 424-440)

        // Zorunlu alanlar
        $zimmet_stok_no = !empty($input_data['zimmet_stok_no']) ? intval($input_data['zimmet_stok_no']) : null;
        $zimmet_departman_no = !empty($input_data['zimmet_departman_no']) ? intval($input_data['zimmet_departman_no']) : null;
        $zimmet_hareket_giris_miktar = !empty($input_data['zimmet_hareket_giris_miktar']) ? intval($input_data['zimmet_hareket_giris_miktar']) : 0;

        if (empty($zimmet_stok_no)) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'zimmet_stok_no gereklidir.'
            ], 400);
        }

        if (empty($zimmet_departman_no)) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'zimmet_departman_no gereklidir.'
            ], 400);
        }

        // Stok kontrolü
        $stok = $this->db->where('zimmet_stok_id', $zimmet_stok_no)->get('zimmet_stoklar')->row();
        if (!$stok) {
            $this->jsonResponse([
                'status'  => 'error',
        ...
        // Departman kontrolü
        $departman = $this->db->where('zimmet_departman_id', $zimmet_departman_no)->get('zimmet_departmanlar')->row();
        if (!$departman) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Geçersiz departman ID.'
            ], 404);
        }

        // Veriyi hazırla
        $data = [
            'zimmet_stok_no' => $zimmet_stok_no,
            'zimmet_departman_no' => $zimmet_departman_no,
            'zimmet_hareket_giris_miktar' => $zimmet_hareket_giris_miktar,
            'zimmet_kullanici_no' => !empty($input_data['zimmet_kullanici_no']) ? intval($input_data['zimmet_kullanici_no']) : 0,
            'zimmet_hareket_cikis_miktar' => 0
        ];

        // Alt bölüm (opsiyonel - üretim için)
        if (!empty($input_data['zimmet_hareket_alt_bolum_no'])) {
            $data['zimmet_hareket_alt_bolum_no'] = intval($input_data['zimmet_hareket_alt_bolum_no']);
        }

        $insert_result = $this->db->insert('zimmet_hareketler', $data);

        if ($insert_result) {
            $hareket_id = $this->db->insert_id();
            
            // Eklenen kaydı getir
            $yeni_hareket = $this->db
                ->select('zh.*, zs.zimmet_stok_adi, d.zimmet_departman_adi, k.kullanici_ad_soyad')
                ->from('zimmet_hareketler zh')
                ->join('zimmet_stoklar zs', 'zh.zimmet_stok_no = zs.zimmet_stok_id', 'left')
                ->join('zimmet_departmanlar d', 'zh.zimmet_departman_no = d.zimmet_departman_id', 'left')
                ->join('kullanicilar k', 'zh.zimmet_kullanici_no = k.kullanici_id', 'left')
                ->where('zh.zimmet_hareket_id', $hareket_id)
                ->get()
                ->row();
            ...
            $this->jsonResponse([
                ...
                'data' => [
                    'zimmet_hareket_id' => intval($hareket_id),
                    'zimmet_stok' => [
                        'zimmet_stok_id' => intval($yeni_hareket->zimmet_stok_no),
                        'zimmet_stok_adi' => $yeni_hareket->zimmet_stok_adi ?? null
                    ],
                    'zimmet_departman' => [
                        'zimmet_departman_id' => intval($yeni_hareket->zimmet_departman_no),
                        'zimmet_departman_adi' => $yeni_hareket->zimmet_departman_adi ?? null
                    ],
                    'zimmet_kullanici_no' => !empty($yeni_hareket->zimmet_kullanici_no) ? intval($yeni_hareket->zimmet_kullanici_no) : null,
                    'kullanici_adi' => $yeni_hareket->kullanici_ad_soyad ?? null,
                    'zimmet_hareket_giris_miktar' => intval($yeni_hareket->zimmet_hareket_giris_miktar),
                    'zimmet_hareket_cikis_miktar' => intval($yeni_hareket->zimmet_hareket_cikis_miktar)
                ],
                'timestamp' => date('Y-m-d H:i:s')
            ]);
        } else {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Zimmet kaydı oluşturulurken bir hata oluştu.'
            ], 500);
        }
    }

    /** 47. Fabrika Zimmet - Güncelle */
    public function fabrika_zimmet_guncelle()
    {
        $method = $this->input->method(true);
        ...
        // Controller Referansı: Zimmet.php::kullaniciya_stok_tanim_guncelle() (satır 474-525)

        // Zimmet hareket ID'sini al
        $zimmet_hareket_id = !empty($input_data['zimmet_hareket_id']) ? intval($input_data['zimmet_hareket_id']) : null;
        
        if (empty($zimmet_hareket_id)) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'zimmet_hareket_id gereklidir.'
            ], 400);
        }

        // Mevcut kaydı kontrol et
        $mevcut_hareket = $this->db->where('zimmet_hareket_id', $zimmet_hareket_id)->get('zimmet_hareketler')->row();
        
        if (!$mevcut_hareket) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Geçersiz zimmet hareket ID.'
            ], 404);
        }
        ...
        if (isset($input_data['zimmet_stok_no'])) {
            $stok_id = intval($input_data['zimmet_stok_no']);
            $stok = $this->db->where('zimmet_stok_id', $stok_id)->get('zimmet_stoklar')->row();
            if (!$stok) {
                $this->jsonResponse([
                    'status'  => 'error',
                    'message' => 'Geçersiz stok ID.'
            ...
            $data['zimmet_stok_no'] = $stok_id;
        }

        if (isset($input_data['zimmet_departman_no'])) {
            $departman_id = intval($input_data['zimmet_departman_no']);
            $departman = $this->db->where('zimmet_departman_id', $departman_id)->get('zimmet_departmanlar')->row();
            if (!$departman) {
                $this->jsonResponse([
                    'status'  => 'error',
                    'message' => 'Geçersiz departman ID.'
            ...
            $data['zimmet_departman_no'] = $departman_id;
        }

        if (isset($input_data['zimmet_kullanici_no'])) {
            $data['zimmet_kullanici_no'] = intval($input_data['zimmet_kullanici_no']);
        }

        if (isset($input_data['zimmet_hareket_giris_miktar'])) {
            $data['zimmet_hareket_giris_miktar'] = intval($input_data['zimmet_hareket_giris_miktar']);
        }

        if (isset($input_data['zimmet_hareket_cikis_miktar'])) {
            $data['zimmet_hareket_cikis_miktar'] = intval($input_data['zimmet_hareket_cikis_miktar']);
        }

        if (isset($input_data['zimmet_hareket_alt_bolum_no'])) {
            $data['zimmet_hareket_alt_bolum_no'] = intval($input_data['zimmet_hareket_alt_bolum_no']);
        }

        // Stok kontrolü (eğer miktar değişiyorsa)
        if (isset($data['zimmet_hareket_giris_miktar']) || isset($data['zimmet_hareket_cikis_miktar'])) {
            $departman_id = $data['zimmet_departman_no'] ?? $mevcut_hareket->zimmet_departman_no;
            $stok_id = $data['zimmet_stok_no'] ?? $mevcut_hareket->zimmet_stok_no;
            $yeni_giris = $data['zimmet_hareket_giris_miktar'] ?? $mevcut_hareket->zimmet_hareket_giris_miktar;
            $yeni_cikis = $data['zimmet_hareket_cikis_miktar'] ?? $mevcut_hareket->zimmet_hareket_cikis_miktar;
            $eski_giris = $mevcut_hareket->zimmet_hareket_giris_miktar;
            $eski_cikis = $mevcut_hareket->zimmet_hareket_cikis_miktar;

            // Toplam stok kontrolü
            $stok_ozet = $this->db
                ->select('SUM(zimmet_hareket_giris_miktar) AS toplam_giris, SUM(zimmet_hareket_cikis_miktar) AS toplam_cikis')
                ->from('zimmet_hareketler')
                ->where('zimmet_stok_no', $stok_id)
                ->where('zimmet_departman_no', $departman_id)
                ->where('zimmet_hareket_id !=', $zimmet_hareket_id)
                ->get()
                ->row();

            $toplam_giris = ($stok_ozet->toplam_giris ?? 0) + $yeni_giris;
            $toplam_cikis = ($stok_ozet->toplam_cikis ?? 0) + $yeni_cikis;
            $kalan = $toplam_giris - $toplam_cikis;

            if ($kalan < 0) {
                $this->jsonResponse([
                    'status'  => 'error',
                    'message' => 'Güncelleme sonrası stok miktarı negatif olamaz. Mevcut stok: ' . ($toplam_giris - ($stok_ozet->toplam_cikis ?? 0))
                ], 400);
            }
        }
        ...
        // Güncelleme işlemi
        $update_result = $this->db->where('zimmet_hareket_id', $zimmet_hareket_id)->update('zimmet_hareketler', $data);

        if ($update_result) {
            // Güncellenmiş veriyi getir
            $guncellenmis_hareket = $this->db
                ->select('zh.*, zs.zimmet_stok_adi, d.zimmet_departman_adi, k.kullanici_ad_soyad')
                ->from('zimmet_hareketler zh')
                ->join('zimmet_stoklar zs', 'zh.zimmet_stok_no = zs.zimmet_stok_id', 'left')
                ->join('zimmet_departmanlar d', 'zh.zimmet_departman_no = d.zimmet_departman_id', 'left')
                ->join('kullanicilar k', 'zh.zimmet_kullanici_no = k.kullanici_id', 'left')
                ->where('zh.zimmet_hareket_id', $zimmet_hareket_id)
                ->get()
                ->row();
            ...
            $this->jsonResponse([
                ...
                'data' => [
                    'zimmet_hareket_id' => intval($zimmet_hareket_id),
                    'zimmet_stok' => [
                        'zimmet_stok_id' => intval($guncellenmis_hareket->zimmet_stok_no),
                        'zimmet_stok_adi' => $guncellenmis_hareket->zimmet_stok_adi ?? null
                    ],
                    'zimmet_departman' => [
                        'zimmet_departman_id' => intval($guncellenmis_hareket->zimmet_departman_no),
                        'zimmet_departman_adi' => $guncellenmis_hareket->zimmet_departman_adi ?? null
                    ],
                    'zimmet_kullanici_no' => !empty($guncellenmis_hareket->zimmet_kullanici_no) ? intval($guncellenmis_hareket->zimmet_kullanici_no) : null,
                    'kullanici_adi' => $guncellenmis_hareket->kullanici_ad_soyad ?? null,
                    'zimmet_hareket_giris_miktar' => intval($guncellenmis_hareket->zimmet_hareket_giris_miktar),
                    'zimmet_hareket_cikis_miktar' => intval($guncellenmis_hareket->zimmet_hareket_cikis_miktar)
                ],
                'timestamp' => date('Y-m-d H:i:s')
            ]);
        } else {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Zimmet kaydı güncellenirken bir hata oluştu.'
            ], 500);
        }
    }

    /** 48. Fabrika Zimmet - Sil */
    public function fabrika_zimmet_sil()
    {
        $method = $this->input->method(true);
        ...
        // Controller Referansı: Zimmet.php::hareket_sil() (satır 225-232)
        ...
        // Silme işlemi
        $delete_result = $this->db->where('zimmet_hareket_id', $zimmet_hareket_id)->delete('zimmet_hareketler');

        if ($delete_result) {
            $this->jsonResponse([
                'status' => 'success',
                'message' => 'Zimmet kaydı başarıyla silindi.',
                'zimmet_hareket_id' => $zimmet_hareket_id,
                'timestamp' => date('Y-m-d H:i:s')
            ]);
        } else {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Zimmet kaydı silinirken bir hata oluştu.'
            ], 500);
        }
    }

    /** 49. Üretim Planlama - Ekle */
    public function uretim_planlama_ekle()
    {
        $method = $this->input->method(true);
        ...
        // Controller Referansı: Uretim_planlama.php::save() (satır 179-283)

        // Zorunlu alanlar
        $urun_fg_id = !empty($input_data['urun_fg_id']) ? intval($input_data['urun_fg_id']) : null;
        $uretim_tarihi = !empty($input_data['uretim_tarihi']) ? trim($input_data['uretim_tarihi']) : null;

        if (empty($urun_fg_id)) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'urun_fg_id gereklidir.'
            ], 400);
        }

        if (empty($uretim_tarihi)) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'uretim_tarihi gereklidir.'
            ], 400);
        }

        // Tarih formatı kontrolü
        if (!strtotime($uretim_tarihi)) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Geçersiz uretim_tarihi formatı. Format: Y-m-d veya Y-m-d H:i:s'
            ], 400);
        }

        // Ürün kontrolü
        $urun = $this->db->where('urun_id', $urun_fg_id)->get('urunler')->row();
        if (!$urun) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Geçersiz ürün ID.'
        ...
        // Renk kontrolü (opsiyonel)
        if (!empty($input_data['renk_fg_id'])) {
            $renk_id = intval($input_data['renk_fg_id']);
            $renk = $this->db->where('renk_id', $renk_id)->where('urun_no', $urun_fg_id)->get('urun_renkleri')->row();
            if (!$renk) {
                $this->jsonResponse([
                    'status'  => 'error',
                    'message' => 'Geçersiz renk ID veya renk bu ürüne ait değil.'
                ], 404);
            }
        }
        ...
        // Veriyi hazırla
        $data = [
            'urun_fg_id' => $urun_fg_id,
            'baslik_bilgisi' => !empty($input_data['baslik_bilgisi']) ? trim($input_data['baslik_bilgisi']) : null,
            'uretim_tarihi' => date('Y-m-d H:i:s', strtotime($uretim_tarihi)),
            'renk_fg_id' => !empty($input_data['renk_fg_id']) ? intval($input_data['renk_fg_id']) : null,
            'kayit_notu' => !empty($input_data['kayit_notu']) ? trim($input_data['kayit_notu']) : null,
            'onay_durumu' => 0, // Varsayılan olarak onay bekliyor
            'aktif_kayit' => 1,
            'kayit_tarihi' => date('Y-m-d H:i:s')
        ];

        // Kullanıcı ID 8 ise otomatik onay
        $kullanici_id = !empty($input_data['kullanici_id']) ? intval($input_data['kullanici_id']) : null;
        if ($kullanici_id == 8) {
            $data['onay_durumu'] = 1;
        }

        $insert_result = $this->db->insert('uretim_planlama', $data);

        if ($insert_result) {
            $uretim_planlama_id = $this->db->insert_id();
            
            // SMS gönderimi (kullanıcı ID 8 değilse)
            if ($kullanici_id != 8) {
            ...
            // Eklenen kaydı getir
            $yeni_plan = $this->db
                ->select('up.*, u.urun_adi, ur.renk_adi')
                ->from('uretim_planlama up')
                ->join('urunler u', 'u.urun_id = up.urun_fg_id', 'left')
                ->join('urun_renkleri ur', 'ur.renk_id = up.renk_fg_id', 'left')
                ->where('up.uretim_planlama_id', $uretim_planlama_id)
                ->get()
                ->row();
            ...
            $this->jsonResponse([
                ...
                'data' => [
                    'uretim_planlama_id' => intval($uretim_planlama_id),
                    'urun' => [
                        'urun_id' => intval($yeni_plan->urun_fg_id),
                        'urun_adi' => $yeni_plan->urun_adi ?? null
                    ],
                    'renk' => !empty($yeni_plan->renk_fg_id) ? [
                        'renk_id' => intval($yeni_plan->renk_fg_id),
                        'renk_adi' => $yeni_plan->renk_adi ?? null
                    ] : null,
                    'baslik_bilgisi' => $yeni_plan->baslik_bilgisi ?? null,
                    'uretim_tarihi' => $yeni_plan->uretim_tarihi ?? null,
                    'kayit_notu' => $yeni_plan->kayit_notu ?? null,
                    'onay_durumu' => intval($yeni_plan->onay_durumu),
                    'onay_durumu_text' => $yeni_plan->onay_durumu == 1 ? 'Onaylandı' : 'Onay Bekliyor',
                    'aktif_kayit' => intval($yeni_plan->aktif_kayit)
                ],
                'timestamp' => date('Y-m-d H:i:s')
            ]);
        } else {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Üretim planı oluşturulurken bir hata oluştu.'
            ], 500);
        }
    }

    /** 50. Üretim Planlama - Güncelle */
    public function uretim_planlama_guncelle()
    {
        $method = $this->input->method(true);
        ...
        // Üretim planlama ID'sini al
        $uretim_planlama_id = !empty($input_data['uretim_planlama_id']) ? intval($input_data['uretim_planlama_id']) : null;
        
        if (empty($uretim_planlama_id)) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'uretim_planlama_id gereklidir.'
            ], 400);
        }

        // Mevcut kaydı kontrol et
        $mevcut_plan = $this->db->where('uretim_planlama_id', $uretim_planlama_id)->get('uretim_planlama')->row();
        
        if (!$mevcut_plan) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Geçersiz üretim planlama ID.'
            ], 404);
        }

        if ($mevcut_plan->aktif_kayit == 0) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Silinen kayıt güncellenemez.'
            ], 400);
        }

        // Güncellenecek verileri hazırla
        $data = [];
        $guncelleme_notu = $mevcut_plan->guncelleme_notu ?? '';
        $smsuyari = '';

        if (isset($input_data['urun_fg_id'])) {
            $urun_id = intval($input_data['urun_fg_id']);
            $urun = $this->db->where('urun_id', $urun_id)->get('urunler')->row();
            if (!$urun) {
                $this->jsonResponse([
                    'status'  => 'error',
            ...
            if ($mevcut_plan->urun_fg_id != $urun_id) {
                $eski_urun = $this->db->where('urun_id', $mevcut_plan->urun_fg_id)->get('urunler')->row();
                $guncelleme_notu .= "<br>Cihaz Bilgisi Güncellendi (" . ($eski_urun->urun_adi ?? '') . " >>" . $urun->urun_adi . " )";
                $smsuyari .= "Cihaz\n";
            }
            $data['urun_fg_id'] = $urun_id;
        }

        if (isset($input_data['renk_fg_id'])) {
            $renk_id = intval($input_data['renk_fg_id']);
            $urun_fg_id = $data['urun_fg_id'] ?? $mevcut_plan->urun_fg_id;
            $renk = $this->db->where('renk_id', $renk_id)->where('urun_no', $urun_fg_id)->get('urun_renkleri')->row();
            if (!$renk) {
                $this->jsonResponse([
                    'status'  => 'error',
            ...
            if ($mevcut_plan->renk_fg_id != $renk_id) {
                $eski_renk = $this->db->where('renk_id', $mevcut_plan->renk_fg_id)->get('urun_renkleri')->row();
                $guncelleme_notu .= "<br>Renk Bilgisi Güncellendi (" . ($eski_renk->renk_adi ?? '') . " >>" . $renk->renk_adi . " )";
                $smsuyari .= "Renk\n";
            }
            $data['renk_fg_id'] = $renk_id;
        }

        if (isset($input_data['baslik_bilgisi'])) {
            $baslik = trim($input_data['baslik_bilgisi']);
            if ($mevcut_plan->baslik_bilgisi != $baslik) {
                $guncelleme_notu .= "<br>Başlık Bilgisi Güncellendi (" . ($mevcut_plan->baslik_bilgisi ?? '') . " >>" . $baslik . " )";
                $smsuyari .= "Başlık Bilgisi\n";
            }
            $data['baslik_bilgisi'] = $baslik;
        }

        if (isset($input_data['uretim_tarihi'])) {
            $uretim_tarihi = trim($input_data['uretim_tarihi']);
            if (!strtotime($uretim_tarihi)) {
                $this->jsonResponse([
                    'status'  => 'error',
                    'message' => 'Geçersiz uretim_tarihi formatı. Format: Y-m-d veya Y-m-d H:i:s'
            ...
            $yeni_tarih = date('Y-m-d', strtotime($uretim_tarihi));
            $eski_tarih = date('Y-m-d', strtotime($mevcut_plan->uretim_tarihi));
            
            if ($eski_tarih != $yeni_tarih) {
                $guncelleme_notu .= "<br>Üretim Tarihi Güncellendi (" . date("d.m.Y", strtotime($mevcut_plan->uretim_tarihi)) . " >>" . date("d.m.Y", strtotime($uretim_tarihi)) . " )";
                $smsuyari .= "Üretim Tarihi\n";
            }
            $data['uretim_tarihi'] = date('Y-m-d H:i:s', strtotime($uretim_tarihi));
        }

        if (isset($input_data['kayit_notu'])) {
            $data['kayit_notu'] = trim($input_data['kayit_notu']);
        }

        // Güncelleme notu varsa ekle
        if (!empty($guncelleme_notu)) {
            $data['guncelleme_notu'] = $guncelleme_notu;
            $data['guncelleme_tarihi'] = date('Y-m-d H:i:s');
            
            // Kullanıcı ID 8 değilse onay durumunu sıfırla
            $kullanici_id = !empty($input_data['kullanici_id']) ? intval($input_data['kullanici_id']) : null;
            if ($kullanici_id != 8) {
                $data['onay_durumu'] = 0;
        ...
        // Güncelleme işlemi
        $update_result = $this->db->where('uretim_planlama_id', $uretim_planlama_id)->update('uretim_planlama', $data);

        if ($update_result) {
            // Güncellenmiş veriyi getir
            $guncellenmis_plan = $this->db
                ->select('up.*, u.urun_adi, ur.renk_adi')
                ->from('uretim_planlama up')
                ->join('urunler u', 'u.urun_id = up.urun_fg_id', 'left')
                ->join('urun_renkleri ur', 'ur.renk_id = up.renk_fg_id', 'left')
            ...
            $this->jsonResponse([
                ...
                'data' => [
                    'uretim_planlama_id' => intval($uretim_planlama_id),
                    'urun' => [
                        'urun_id' => intval($guncellenmis_plan->urun_fg_id),
                        'urun_adi' => $guncellenmis_plan->urun_adi ?? null
                    ],
                    'renk' => !empty($guncellenmis_plan->renk_fg_id) ? [
                        'renk_id' => intval($guncellenmis_plan->renk_fg_id),
                        'renk_adi' => $guncellenmis_plan->renk_adi ?? null
                    ] : null,
                    'baslik_bilgisi' => $guncellenmis_plan->baslik_bilgisi ?? null,
                    'uretim_tarihi' => $guncellenmis_plan->uretim_tarihi ?? null,
                    'kayit_notu' => $guncellenmis_plan->kayit_notu ?? null,
                    'guncelleme_notu' => $guncellenmis_plan->guncelleme_notu ?? null,
                    'onay_durumu' => intval($guncellenmis_plan->onay_durumu),
                    'onay_durumu_text' => $guncellenmis_plan->onay_durumu == 1 ? 'Onaylandı' : 'Onay Bekliyor',
                    'aktif_kayit' => intval($guncellenmis_plan->aktif_kayit)
                ],
                'timestamp' => date('Y-m-d H:i:s')
            ]);
        } else {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Üretim planı güncellenirken bir hata oluştu.'
            ], 500);
        }
    }

    /** 51. Üretim Planlama - Sil */
    public function uretim_planlama_sil()
    {
        $method = $this->input->method(true);
        ...
        // Controller Referansı: Uretim_planlama.php::delete() (satır 140-148)
        ...
        // Silme işlemi (aktif_kayit = 0 yaparak soft delete)
        $delete_result = $this->db->where('uretim_planlama_id', $uretim_planlama_id)->update('uretim_planlama', ['aktif_kayit' => 0]);

        if ($delete_result) {
            $this->jsonResponse([
                'status' => 'success',
                'message' => 'Üretim planı başarıyla silindi.',
                'uretim_planlama_id' => $uretim_planlama_id,
                'timestamp' => date('Y-m-d H:i:s')
            ]);
        } else {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Üretim planı silinirken bir hata oluştu.'
            ], 500);
        }
    }

    /** 52. Üretim Planlama - Onay */
    public function uretim_planlama_onay()
    {
        $method = $this->input->method(true);
        ...
        // Controller Referansı: Uretim_planlama.php::onay() (satır 151-162)

        // Üretim planlama ID'sini al
        $uretim_planlama_id = !empty($input_data['uretim_planlama_id']) ? intval($input_data['uretim_planlama_id']) : null;
        $onay_durumu = isset($input_data['onay_durumu']) ? intval($input_data['onay_durumu']) : 1; // 1: Onayla, 0: Onayı Geri Al
        ...
        if ($mevcut_plan->aktif_kayit == 0) {
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Silinen kayıt onaylanamaz.'
            ], 400);
        }

        // Onay durumu güncelleme
        $update_result = $this->db->where('uretim_planlama_id', $uretim_planlama_id)->update('uretim_planlama', ['onay_durumu' => $onay_durumu]);
        ...
        if ($update_result) {
            ...
            $this->jsonResponse([
                'status' => 'success',
                'message' => $onay_durumu == 1 ? 'Üretim planı başarıyla onaylandı.' : 'Üretim planı onayı başarıyla geri alındı.',
                'data' => [
                    'uretim_planlama_id' => intval($uretim_planlama_id),
                    'onay_durumu' => intval($onay_durumu),
                    'onay_durumu_text' => $onay_durumu == 1 ? 'Onaylandı' : 'Onay Bekliyor',
                    'urun' => [
                        'urun_id' => intval($guncellenmis_plan->urun_fg_id),
                        'urun_adi' => $guncellenmis_plan->urun_adi ?? null
                    ]
            ...
            $this->jsonResponse([
                'status'  => 'error',
                'message' => 'Üretim planı onay durumu güncellenirken bir hata oluştu.'
            ], 500);
        }
    }
