...
$filter_merkez_adresi = ((strlen($row->merkez_adresi) > 50) ? mb_substr($row->merkez_adresi, 0, 40) . '...': $row->merkez_adresi) ;
            ...
            $data[] = [ 
              ...
              "<span style='font-weight:normal'><b>".' <i class="fas fa-map-marked-alt" style="color: #178018;"></i> '.$row->merkez_adi."</b> ". '<a type="button"  onclick="showWindow(\''.base_url("merkez/duzenle/".$row->merkez_id).'\');" target="_blank" class="text-orange" style="font-size: 12px!important;font-weight:normal;"> Düzenle</a>'."<br>Sipariş Kodu : ".'<a class="text-primary" style="cursor:pointer" onclick="showDetail(\''.$urlcustom.'/1\')">'.(($row->satis_fiyati > 0) ? $row->siparis_kodu : "<span style='opacity:0.5;color:black!important'>Sistem Öncesi Kayıt / Sipariş Yok</span>")."</a>".($row->takas_bedeli > 0 ? " <span style='color: red;'>(Takaslı)</span>" : "")."</span>"
              ...
              "<span title='$row->merkez_adresi' style='font-weight:normal'><b>".$row->sehir_adi." / ".$row->ilce_adi."</b><br>".(($row->merkez_adresi == "" || $row->merkez_adresi == "0" || $row->merkez_adresi == ".")?"<span style='opacity:0.4'>BU MERKEZE TANIMLI ADRES KAYDI BULUNAMADI</span>":$filter_merkez_adresi)."</span>"
              .($row->merkez_kayit_guncelleme_notu != "" ? '<br><div style=" background: #03ff351c; border: 1px solid #00b324; border-radius: 3px; padding: 2px; color: green; "><i class="fas fa-check-circle"></i><b style="font-weight: 490;"> Güncellendi : </b><span style="font-weight:normal"> '.$row->merkez_kayit_guncelleme_notu.'</span></div>' : "")
