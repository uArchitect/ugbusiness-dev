        ...
        foreach ($query->result() as $row) {
            ...
            if(date("Y-m-d",strtotime($row->garanti_bitis_tarihi)) == date("Y-m-d",strtotime($row->garanti_baslangic_tarihi))){
                ...
                if(date("Y-m-d",strtotime($row->garanti_bitis_tarihi)) < date("Y-m-d")){
                   ...
                   $musteri = '<a target="_blank" style="font-weight:normal" href="https://ugbusiness.com.tr/musteri/profil/'.$row->musteri_id.'"><i class="fa fa-user-circle" style="color: #035ab9;"></i> '.$row->musteri_ad.'</a>';     
            ...
            $data[] = [ 
              ...
              "<span style='font-weight:normal'><b>".$row->merkez_adi."</b> "."<br>Sipariş Kodu : "."<a href=".$urlcustom.">".$row->siparis_kodu."</a>".($row->takas_bedeli > 0 ? " <span style='color: red;'>(Takaslı)</span>" : "")."</span>",
