...
$urlcustom = base_url("siparis/report/").urlencode(base64_encode("Gg3TGGUcv29CpA8aUcpwV2KdjCz8aE".$row->siparis_id."Gg3TGGUcv29CpA8aUcpwV2KdjCz8aE"));
            ...
            $data[] = [ 
              ...
              "<span style='font-weight:normal'><b>".' <i class="fa fa-building" style="color: #ff6c00;"></i> '.$row->merkez_adi."</b> ". '<a type="button"  onclick="showWindow(\''.base_url("merkez/duzenle/".$row->merkez_id).'\');" target="_blank" class="text-orange" style="font-size: 12px!important;font-weight:normal;"> Düzenle</a>'."<br>Sipariş Kodu : ".'<a class="text-primary" style="cursor:pointer" onclick="showDetail(\''.$urlcustom.'/1\')">'.(($row->satis_fiyati > 0) ? $row->siparis_kodu : "<span style='opacity:0.5;color:black!important'>Eski Kayıt / Sipariş Yok</span>")."</a>".($row->takas_bedeli > 0 ? " <span style='color: red;'>(Takaslı)</span>" : "")."</span>"
