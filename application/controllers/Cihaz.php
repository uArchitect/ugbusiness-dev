...
$filter_merkez_adresi = ((strlen($row->merkez_adresi) > 50) ? mb_substr($row->merkez_adresi, 0, 40) . '...': $row->merkez_adresi) ;
            ...
            $data[] = [ 
              ...
              "<span style='font-weight:normal'><b>".' <i class="fa fa-building" style="color: #ff6c00;"></i> '.($row->merkez_adi != "#NULL#" ? $row->merkez_adi : '<span class="badge bg-danger" style="background: #ffd1d1 !important; color: #b30000 !important; border: 1px solid red;"><i class="nav-icon 	fas fa-exclamation-circle"></i> Merkez Adı Girilmedi</span>')."</b> ". '<a type="button"  onclick="showWindow(\''.base_url("merkez/duzenle/".$row->merkez_id).'\');" target="_blank" class="btn btn-xs btn-warning p-0 pl-1 pr-1" style="font-size: 12px!important;font-weight:normal;"> Düzenle</a>'."<br>Sipariş Kodu : ".'<a class="text-primary" style="cursor:pointer" onclick="showDetail(\''.$urlcustom.'/1\')">'.(($row->satis_fiyati > 0) ? $row->siparis_kodu : "<span style='opacity:0.5;color:black!important'>Sistem Öncesi Kayıt / Sipariş Yok</span>")."</a>".($row->takas_bedeli > 0 ? " <span style='color: red;'>(Takaslı)</span>" : "")."</span>"
