  ...
  <div class="flex flex-col items-center gap-2 lg:gap-3.5 py-4 lg:pt-5 lg:pb-10">
   ...
   <div class="flex flex-wrap justify-center gap-1 lg:gap-4.5 text-sm">
    ...
    <div class="flex gap-1.25 items-center">
     ...
     <span class="text-gray-600 font-medium"><?=htmlspecialchars($kullanici->ugajans_kullanici_telefon)?></span>
    </div>
    <?php endif; ?>
    ...
    <div class="flex gap-1.25 items-center">
     ...
     <span class="text-gray-600 font-medium"><?=htmlspecialchars($kullanici->ugajans_kullanici_adi ?? '')?></span>
    </div>
   </div>
  </div>
 </div>
 ...
 <div class="grid grid-cols-1 gap-5 lg:gap-7.5">
  ...
  <div class="card">
   ...
   <div class="card-footer justify-end">
    <button type="submit" form="fotografForm" class="btn btn-primary btn-sm">
  ...
  <div class="card">
     ...
     <div class="grid gap-4">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center gap-3">
       ...
       <div class="flex-1">
        ...
        <small class="text-xs text-gray-500 mt-1 block">Değiştirilemez</small>
       </div>
      </div>
   ...
   <div class="card-footer justify-end">
    <button type="submit" form="profilForm" class="btn btn-primary btn-sm">
     <i class="ki-filled ki-check"></i>
     Kaydet
    </button>
   </div>
  ...
  <div class="card">
     ...
     <div class="grid gap-4">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center gap-3">
       ...
       <div class="flex-1">
        ...
        <small class="text-xs text-gray-500 mt-1 block">En az 6 karakter olmalıdır</small>
       </div>
      </div>
   ...
   <div class="card-footer justify-end">
    <button type="submit" form="sifreForm" class="btn btn-success btn-sm">
     <i class="ki-filled ki-check"></i>
     Güncelle
    </button>
   </div>
 ...
 if (input.files && input.files[0]) {
  ...
  if (input.files[0].size > maxSize) {
   if (typeof Swal !== 'undefined') {
    Swal.fire({icon: 'error', title: 'Dosya Çok Büyük', text: 'Profil fotoğrafı maksimum 2MB olabilir.'});
   } else {
    alert('Profil fotoğrafı maksimum 2MB olabilir.');
   }
   input.value = '';
   return;
  }
  ...
  if (!allowedTypes.includes(input.files[0].type)) {
   if (typeof Swal !== 'undefined') {
    Swal.fire({icon: 'error', title: 'Geçersiz Dosya Tipi', text: 'Sadece JPG, PNG ve GIF formatları desteklenmektedir.'});
   } else {
  ...
  reader.onerror = function() {
   if (typeof Swal !== 'undefined') {
    Swal.fire({icon: 'error', title: 'Hata', text: 'Dosya okunurken bir hata oluştu.'});
   } else {
 ...
 if (fotografForm) {
  fotografForm.addEventListener('submit', function(e) {
   ...
   if (!fotografInput || !fotografInput.files || fotografInput.files.length === 0) {
    e.preventDefault();
    if (typeof Swal !== 'undefined') {
     Swal.fire({icon: 'error', title: 'Dosya Seçilmedi', text: 'Lütfen yeni bir fotoğraf seçin.'});
    } else {
 ...
 if (sifreForm) {
  sifreForm.addEventListener('submit', function(e) {
   ...
   if (!mevcutSifre) {
    e.preventDefault();
    if (typeof Swal !== 'undefined') {
     Swal.fire({icon: 'error', title: 'Eksik Bilgi', text: 'Şifre değiştirmek için mevcut şifrenizi girmelisiniz.'});
    } else {
   ...
   if (!yeniSifre || !yeniSifreTekrar) {
    e.preventDefault();
    if (typeof Swal !== 'undefined') {
     Swal.fire({icon: 'error', title: 'Eksik Bilgi', text: 'Yeni şifre alanlarını doldurmalısınız.'});
    } else {
   ...
   if (yeniSifre !== yeniSifreTekrar) {
    e.preventDefault();
    if (typeof Swal !== 'undefined') {
     Swal.fire({icon: 'error', title: 'Şifreler Eşleşmiyor', text: 'Yeni şifreler birbiriyle eşleşmiyor.'});
    } else {
   ...
   if (yeniSifre.length < 6) {
    e.preventDefault();
    if (typeof Swal !== 'undefined') {
     Swal.fire({icon: 'error', title: 'Şifre Çok Kısa', text: 'Yeni şifre en az 6 karakter olmalıdır.'});
    } else {
