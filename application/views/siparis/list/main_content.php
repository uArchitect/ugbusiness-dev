    ...
    <table id="onaybekleyensiparisler" class="table table-bordered table-striped nowrap">
      ...
      <?php $count=0; foreach ($onay_bekleyen_siparisler as $siparis) : ?>
        ...
        <tr onclick="location.href='<?=$link?>';" style="cursor:pointer;">
          ...
          <td>
            <b><i class="far fa-building" style="color: green;"></i> <?=($siparis->merkez_adi == "#NULL#") ? "<span class='badge bg-danger'>Merkez Adı Girilmedi</span>":$siparis->merkez_adi?> </b>   <br>  <?=$siparis->sehir_adi?> / <?=$siparis->ilce_adi?> 
          </td>           
          <td>
            <b>
          ...
          <td>
            <a type="button" style="margin-top: 10px;" href="<?=base_url("siparis/report/").urlencode(base64_encode("Gg3TGGUcv29CpA8aUcpwV2KdjCz8aE".$siparis->siparis_id."Gg3TGGUcv29CpA8aUcpwV2KdjCz8aE"))?>" onclick="waiting('Sipariş Detayları');" class="btn btn-warning btn-xs"><i class="fas fa-search" style="font-size:14px" aria-hidden="true"></i> <b>SİPARİŞİ GÖRÜNTÜLE</b></a>
          </td>
        </tr>
        <?php  endforeach; ?>
        </tbody>
