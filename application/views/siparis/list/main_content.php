...
<div class="card card-warning" style="border-radius:0px !important;">
  ...
  <div class="card-body" style="margin-top: -12px;margin-left: -12px;">
    ...
    <table id="onaybekleyensiparisler" class="table table-bordered table-striped nowrap">
      ...
      <?php $count=0; foreach ($onay_bekleyen_siparisler as $siparis) : ?>
        ...
        <tr onclick="location.href='<?=$link?>';" style="cursor:pointer;">
          ...
          <td>
            <b> <?=($siparis->merkez_adi == "#NULL#") ? "<span class='badge bg-danger' style='background: #ffd1d1 !important; color: #b30000 !important; border: 1px solid red;'><i class='nav-icon 	fas fa-exclamation-circle'></i> Merkez Adı Girilmedi</span>":'<i class="far fa-building" style="color: green;"></i> '.$siparis->merkez_adi?> </b>   <br>  <?=$siparis->sehir_adi?> / <?=$siparis->ilce_adi?> 
          </td>           
          <td>
            <b>
    ...
    padding-top: 13px;border: 1px solid #5b4002;    font-weight: 400!important;" href="<?=base_url("siparis/report/").urlencode(base64_encode("Gg3TGGUcv29CpA8aUcpwV2KdjCz8aE".$siparis->siparis_id."Gg3TGGUcv29CpA8aUcpwV2KdjCz8aE"))?>" onclick="waiting('Sipariş Detayları');" class="btn btn-warning btn-xs"><i class="fas fa-search" style="font-size:14px" aria-hidden="true"></i> <b>SİPARİŞİ GÖRÜNTÜLE</b></a>
          </td>
        </tr>
        <?php  endforeach; ?>
        </tbody>
