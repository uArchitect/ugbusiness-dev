 ...
 Siparişe tanımlı toplam <?=count($urunler)?> adet ürün listelenmiştir.
                ...
                <table id="tableurunlersf" class="table table-striped" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                    ...
                    if($siparis_fiyat_goruntule){
                      ...
                      <th style="padding-top:5px;padding-bottom:5px;font-weight:normal; color:white;background: #00347d;border-bottom:0px solid">Damla Etiket / Takas Bedeli</th>
...
<?php
    ...
    border: 0;color: #ff0000; margin-left: 7px;" onclick="silOnayla('<?= $silmeUrl ?>')">X</button>
                                        ...
                                        <?php 
                                          // Takas bedeli kolonu kaldırıldı - artık damla etiket ve hediye kolonlarında gösterilecek
                                        ?>  
...
<?php 
                                        ...
                                        <?php
                                        // Damla Etiket
                                        echo (($urun->damla_etiket == 1) ? "<span class='badge bg-default  text-success' style='background: #d6ebd1;padding:5px;font-weight:normal'><i class='fa fa-check-circle text-success'></i> EVET / YAPILACAK</span>" : "<span style='background: #ffdddd;padding:5px;font-weight:normal' class='badge bg-default  text-danger'><i class='fa fa-times-circle text-danger'></i> HAYIR / YAPILMAYACAK</span>");
                                        
                                        // Takas Bedeli (damla etiket kolonuna eklendi)
                                        if($siparis_fiyat_goruntule && $urun->takas_bedeli > 0){
                                            echo "<br><br><strong style='color:#b30000'>Takas Bedeli:</strong><br>";
                                            echo "<span style='font-weight:bold;color:#b30000'>".number_format($urun->takas_bedeli,2)."$paraicon</span>";
                                            if($urun->takas_alinan_seri_kod){
                                                echo "<br><small style='color:#666'>Seri: ".$urun->takas_alinan_seri_kod."</small>";
                                            }
                                        }
                                        ?>
                                          </td>
                                        ...
                                        if($siparis_fiyat_goruntule){
                                            if($urun->takas_bedeli > 0){
                                                ...
                                                if($urun->takas_alinan_model){
                                                    echo "Model: ".$urun->takas_alinan_model;
                                                    if($urun->takas_alinan_renk){
                                                        echo " (".$urun->takas_alinan_renk.")";
                                                    }
                                                    echo "<br>";
                                                }
